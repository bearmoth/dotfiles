...
A session-level fact and, since ADR-0008, a **transitional backstop** rather than the primary safety mechanism. Governance is now stateless read/write ACLs over context × privacy: cross-context reads of a `private: true` vault are hard-denied, and org-vault writes are same-context-only. Taint remains only for the one surviving private-read-then-org-write path (the draining Easygo vault): it is set **only by an actual same-context read** from a `private: true` vault — writes *into* a private vault (worklog, capture) never taint (fixing the directionality bug that tripped the gate on the end-of-session worklog routine). Once set, later autonomous writes to an org-exposed destination are human-gated for the session. It records that private material *entered the session*, never which claims derive from it, and cannot be shed by delegating the write to a fresh agent (the delegation carries the provenance). Retired on data once the Easygo vault migrates (ADR-0008). Contrast with: *promotion* (the human-gated act taint forces), per-claim provenance (rejected — it would need the nuance-judging the exposure model forbids).
