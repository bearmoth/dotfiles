#!/bin/bash
# Tide prompt configuration. Edit this file to update your prompt settings;
# chezmoi will re-apply it on next `chezmoi apply`.
fish << 'FISH'

# Fish key bindings — must be explicit so Tide shows ❯ not ❮
set -U fish_key_bindings fish_default_key_bindings

# Prompt layout
set -U tide_left_prompt_items pwd git newline character
set -U tide_right_prompt_items status cmd_duration context jobs direnv bun node python rustc java php pulumi ruby go gcloud distrobox toolbox terraform aws nix_shell crystal elixir zig
...
# Context (user@host — hidden by default, shown over SSH)
...
# Node / Bun / Python / Ruby / Go / Rust / Java / PHP / Elixir / Crystal / Zig
...
set -U tide_docker_default_contexts default colima docker-desktop
...
# Vi mode indicator (shown in right prompt if vi bindings are active)
